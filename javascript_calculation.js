// CRC-16/ARC in table form per http://cpsr.org/prevsite/cpsr/privacy/crypto/tools/other/crc-16.c/view
var crcArc16Table = [
 ...
 * @return {number}
 */
function crc16ArcStr(str) {
    var ix;
    var crc = 0;
    for (ix = 0; ix < str.length; ix++) {
        crc = crcArc16Table[(crc ^ str.charCodeAt(ix)) & 0xFF] ^ ((crc >> 8) & 0xFF);
    }
    return crc;
}

let number = crc16ArcStr(window.location.href.split("#").pop().toLowerCase());
var recalcd = false;
if (number < 1024) {
    // port numbers under 1024 need 'root' privileges, so we should avoid them
    number = number + 1024;
    recalcd = true;
}